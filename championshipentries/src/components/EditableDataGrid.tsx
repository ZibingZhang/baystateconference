...
function resolvePasteValue(api: GridApiCommon, field: string, text: string): unknown {
  ...
  if (!column || column.type !== 'singleSelect' || !('valueOptions' in column) || !column.valueOptions) {
    return text
  }
...
function EditableDataGrid<T extends { id: string }>({
  ...
  const pendingCopyRangeRef = useRef<{ grid: string[][]; rowIds: GridRowId[]; fields: string[] } | null>(null)
  ...
  useEffect(() => {
    ...
    const unsubOver = api.subscribeEvent(
      'cellMouseOver',
  ...
  const handleCellKeyDown = (
    ...
    if (event.key === 'Escape' && params.cellMode !== 'edit' && (copiedCells.size > 0 || selection.size > 0)) {
    ...
    if (isArrowKey && params.cellMode !== 'edit') {
      ...
      if (event.shiftKey) {
        ...
        if (event.metaKey || event.ctrlKey) {
          ...
          nextRowIndex = isVertical ? findBlockEdgeIndex(isEmptyAt, rowIds.length, rowIndex, step) : rowIndex
          nextColIndex = isVertical ? colIndex : findBlockEdgeIndex(isEmptyAt, fields.length, colIndex, step)
        } else {
          nextRowIndex = isVertical ? rowIndex + step : rowIndex
          nextColIndex = isVertical ? colIndex : colIndex + step
        }
        if (nextRowIndex < 0 || nextRowIndex >= rowIds.length || nextColIndex < 0 || nextColIndex >= fields.length) {
          return
        }
  ...
  const copiedCellSides = (id: GridRowId, field: string): string => {
    ...
    const hasNeighbor = (r: number, c: number) =>
      r >= 0 && r < rowIds.length && c >= 0 && c < fields.length && copiedCells.has(cellKey(rowIds[r], fields[c]))
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <Box ref={containerRef} sx={{ flex: 1, minHeight: 0 }} onCopy={handleCopy} onPaste={handlePaste}>
        <Box
          component="textarea"
