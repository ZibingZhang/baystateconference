...
export function writeField(
  ...
  const padded = justify === 'left' ? truncated.padEnd(length, ' ') : truncated.padStart(length, ' ')
  const start = start1Based - 1
 ...
 */
export function parseOptionalNumericTime(raw: string, opts: { allowNegative?: boolean } = {}): number | undefined {
  const v = raw.trim()
